...
Defined at [src/lifecycle.gd:87](https://github.com/ametrocavich/vostok-mod-loader/blob/development/src/lifecycle.gd#L87). Outline:
...
`defer_activation=true` on the first-time pack generation is deliberate: without it, activation runs against the already-pinned PCK bytecode in this engine process, fires a misleading `"hooks WILL NOT fire this session"` STABILITY alarm, then restarts anyway. Passing `defer_activation=true` writes the zip + pass state and lets Pass 2's fresh engine mount it cleanly. See [hook_pack.gd:500](https://github.com/ametrocavich/vostok-mod-loader/blob/development/src/hook_pack.gd#L500).
...
`override.cfg` is written directly to the game directory (not `user://`) because Godot reads it at engine startup before any script runs. Canonical layout (written by `_write_override_cfg` at [boot.gd:469](https://github.com/ametrocavich/vostok-mod-loader/blob/development/src/boot.gd#L469)):
...
- **ModLoader is always in `[autoload_prepend]`, last entry.** `[autoload_prepend]` is reverse-insertion -- last listed = first loaded. This ensures ModLoader's static-init mount runs before any mod autoload's script references resolve. See the rationale at [boot.gd:475-480](https://github.com/ametrocavich/vostok-mod-loader/blob/development/src/boot.gd#L475).
...
- **Atomic write via `.tmp` + rename.** [boot.gd:488-502](https://github.com/ametrocavich/vostok-mod-loader/blob/development/src/boot.gd#L488). `DirAccess.rename()` on Windows won't overwrite, so the target is removed first.
...
Writer: `_write_pass_state` at [boot.gd:524](https://github.com/ametrocavich/vostok-mod-loader/blob/development/src/boot.gd#L524). Hash computed at [boot.gd:544](https://github.com/ametrocavich/vostok-mod-loader/blob/development/src/boot.gd#L544).

Including `modloader.gd`'s own mtime in the hash is load-bearing ([boot.gd:563-577](https://github.com/ametrocavich/vostok-mod-loader/blob/development/src/boot.gd#L563)): `ProjectSettings.load_resource_pack` dedupes by path, so rebuilding the loader without a restart would leave the old hook pack mount active with stale file offsets.
...
- **Restart counter** in pass state, incremented by `_write_pass_state`. `_check_crash_recovery` at [boot.gd:590](https://github.com/ametrocavich/vostok-mod-loader/blob/development/src/boot.gd#L590) force-resets when it hits `MAX_RESTART_COUNT` (2).
- **Safe mode file** (`<exe>/modloader_safe_mode`) -- user-placed. `_check_safe_mode` at [boot.gd:605](https://github.com/ametrocavich/vostok-mod-loader/blob/development/src/boot.gd#L605) detects, wipes state, removes the sentinel.
