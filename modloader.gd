...
const API_CHECK_TIMEOUT := 15.0         # seconds for API requests
...
func _ready() -> void:
	...
	_load_ui_config()
	await _show_mod_ui()
	# Save user's enable/disable choices from the UI
	_save_ui_config()
	# Re-scan mods folder to pick up any mods installed via the Browse tab
	_ui_mod_entries = _collect_mod_metadata()
	# Reload the saved config onto the fresh entries
	_load_ui_config()
	_load_all_mods()
	if _developer_mode:
...
func _show_mod_ui() -> void:
	...
	var browse_tab := _build_browse_tab(rebuild_mods_tab)
...
func _build_browse_tab(rebuild_mods_tab: Callable = Callable()) -> Control:
...
func _browse_fetch(query: String, page: int, cat_id: int = 0) -> void:
	...
	if _browse_installed_only:
		# Build cards from local data without API call
...
func _browse_add_mod_card(mod: Dictionary) -> void:
	...
	if is_installed and remote_version != "":
		...
		if local_ver != "" and _compare_versions(local_ver, remote_version) < 0:
			needs_update = true
	...
	if is_installed and needs_update:
		...
		update_btn.pressed.connect(func():
			...
			var ok := await _download_and_replace_mod(target, mod_id)
			if ok:
